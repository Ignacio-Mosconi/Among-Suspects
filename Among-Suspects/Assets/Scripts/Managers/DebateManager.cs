...
public class DebateManager : MonoBehaviour
{
    ...
    {
        if (!String.IsNullOrEmpty(dialogueSound.eventName))
        {
            string eventName = dialogueSound.eventName;
            float playDelay = dialogueSound.playDelay;
            playSoundOnFocusFinishAction = () => PlayDialogueSoundOnFocusFinish(eventName, playDelay);

            debateCameraController.OnFocusFinish.AddListener(playSoundOnFocusFinishAction);
        }
    }

    void PlayDialogueSoundOnFocusFinish(string eventName, float delay)
    {
        AudioManager.Instance.PostEventDelayed(eventName, delay);
    ...
    void Argue(DebateDialogue debateDialogue)
    {
        ...
        {
            if (!String.IsNullOrEmpty(debateDialogue.dialogueSound.eventName))
                AudioManager.Instance.PostEventDelayed(debateDialogue.dialogueSound.eventName, debateDialogue.dialogueSound.playDelay);
